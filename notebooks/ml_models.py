...
def _(LEAKAGE_COLS, TARGET, ml_df, pd):
    ...
    return X, binary_features, continuous_features, ordinal_features, y
...
def _(X, column, np, pd, y):
    ...
    top_binary_screen_df = binary_screen_df.head(20)
...
@app.cell
def _(Path, X, continuous_features, leaves_list, linkage, plt, sns):
...
def _(mo):
    ...
    Scale continuous numeric features for linear models. Leave tree-based models on the raw numeric matrix.
    """)
    return


@app.cell
def _(binary_features, continuous_features, mo, ordinal_features):
    scaling_policy = mo.md(
        f"""
        - Linear models: scale `{len(continuous_features) + len(ordinal_features)}` continuous/ordinal features and passthrough `{len(binary_features)}` binary features.
        - Tree models: no scaling needed.
        - Future target switch: replace `accuracy` with `confidence` and reuse the same analysis cells.
        """
    )
    return
